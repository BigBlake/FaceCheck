...
from db.GetFaceFeaturesDb import GetFaceFeaturesDb
...
# 从数据库中取人脸特征
def getDbFaceFeatures():
    getFaceDb = GetFaceFeaturesDb()
    rows = getFaceDb.getFace()
    # (1, 'sxf', '-0.05827213334850967,0.10025314753875136,0.00511124002514407')
    for row in rows:
        faceList=[]
        faces=row[2].split(",")
        for face in faces:
            faceList.append(float(face))
        features_known_arr.append(faceList)  # 取特征
        userid_known_arr.append(row[0])  # 取特征
        name_known_arr.append(row[1])  # 取特征
        ...
        if len(faces) != 0:
            ...
            for k in range(len(faces)):
                ...
                if min(e_distance_list) < 0.4:
                    ...
                    name_namelist[k] = name_known_arr[int(similar_person_num)]
                    print("May be person " +  name_known_arr[int(similar_person_num)])
                else:
